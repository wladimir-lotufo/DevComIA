...
- Inputs: 
    ...
    3. **Entity Definition**: `docs/ai/design/data\{TableName}.md` (Context).
    4. **Current Scripts**: `infrastructure/sql\Scripts.sql` (to verify fields and relationships to be used).

- Objective: Create or update `APIs/{TableName}/Db{TableName}.cs` and associated Mod files.

- Steps:
    1.  **Analyze the Table/Entity**:
        - Locate the table definition in `infrastructure/sql/Scripts.sql` or `docs/ai/design/data/{TableName}.md`.
    ...
    3.  **Implement CRUD Methods**:
        - For each operation (Insert, GetById, Update, Delete, ListAll), generate:
            - **Simplex Notation**: Inside `/// <summary>` blocks with a ` ```simplex ` block, strictly following the format in `Simplex.md` and examples in `.mentor/knowledge/SampleWorkfolder/APIs/Usuario/DbUsuario.cs`.
            - **C# Method**: Implement the method matching the Simplex signature.
                ...
                - Follow the exact coding style of `.mentor/knowledge/SampleWorkfolder/APIs/Usuario/DbUsuario.cs`.
    ...
    5. **Automated Testing (Mandatory)**:
        ...
        - Generate Unit Tests and Integration Tests for all Dal methods and Models.
