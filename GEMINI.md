...
> Este arquivo define como a Inteligência Artificial deve se comportar neste workspace. Ele possui prioridade MÁXIMA.

---

## 🌟 TREINAMENTOS PERSONALIZADOS E WORKFLOWS ADICIONAIS

> **MANDATORY:** Leia as regras prioritárias descritas no arquivo `MENTOR.md` localizado na raiz do projeto. As instruções contidas em `MENTOR.md` sobre a pasta `@[.mentor]` atuam como extensão primária a este arquivo e possuem prioridade máxima de sobreposição ou adição de regras ao kit base (`.agent`).
